...
function Calendar(id) {
  var input = $("#" + id).attr("autocomplete", "off")
      .css("margin-bottom", '0');
  var date;
  var table;
  ...
  tableDiv.on('click', 'tr.month', function(event) {
    var target = $(event.target);

    if (target.attr('id') == 'next-month') {
      console.log('hello');
      date = new Date(date.getFullYear(), date.getMonth() + 1);
      table = $(getCalendarTable(date));
      tableDiv.html(table);
      var todayCell = $('.today-cell');
      todayCell.removeClass('today-cell');
    } else if (target.attr('id') == 'prev-month') {
      date = new Date(date.getFullYear(), date.getMonth() - 1);
      table = $(getCalendarTable(date));
      tableDiv.html(table);
      $('.today-cell').removeClass('today-cell');
      $('.selected-cell').removeClass('selected-cell');
    }
  ...
  $('html').click(function(event) {
    if (event.target.id != id && tableDiv.is(':visible')) {
      tableDiv.hide();
    }
  });
  ...
  function getCalendarTable(date) {
    ...
    var table = '<table><tr class="month"><th id="prev-month">&laquo</th>'
              + '<th colspan="5">' + getMonthName(month)
              + '</th><th id="next-month">&raquo</th></tr>'
